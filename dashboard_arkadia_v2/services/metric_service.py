        ...
        try:
            ...
            for i in range(len(last_fridays) - 1):
                ...
                if start_balance and end_balance and start_balance.value_usd > Decimal('0.0'):
                    ...
                    logger.debug(f"Strategy: {strategy.name}, Start Date: {start_date}, End Date: {end_date}")
                    logger.debug(f"Start Balance: {start_balance.value_usd}, End Balance: {end_balance.value_usd}")
                    logger.debug(f"Deposits: {deposits}, Withdrawals: {withdrawals}")
                    logger.debug(f"Adjusted Start Value: {adjusted_start_value}, Monthly Performance: {monthly_performance}")
                else:
